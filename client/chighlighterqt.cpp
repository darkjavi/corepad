...
// protected methods
void cHighlighterQT::highlightBlock(const QString &text)
{
...
// public methods
cHighlighterQT::cHighlighterQT(QTextDocument *pParent) :
    cHighlighterCPlusPlus(pParent)
{
    ...
    // directives...
    m_directiveFormat.setForeground(Qt::darkGreen);
    QStringList directivePatterns;
    directivePatterns << "Q_OBJECT";
    foreach (const QString &pattern, directivePatterns)
    {
      rule.m_pattern = QRegExp(pattern);
      rule.m_format = m_directiveFormat;
      m_highlightingRules.append(rule);
    }
    ...
    keywordPatterns << "\\bbreak\\b" << "\\bcase\\b"
                    ...
                    << "\\breturn\\b"
                    << "\\bshort\\b" << "\\bsignals\\b" << "\\bsigned\\b" << "\\bsizeof\\b"
